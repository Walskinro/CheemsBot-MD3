...
console.log(color(figlet.textSync('ALVYN-BOTZ-MD', {
		font: 'Pagga',
...
console.log(color('GITHUB: alvyn-vengeance ','silver'))
console.log(color('INSTAGRAM: vynutzxx22','silver'))
console.log(color('🐶 WA NUMBER: +62859193578139 ','silver'))
console.log(color('  🦄ALVYN-BOTZ 2022🦄','mediumseagreen'))
    ...
    console.log(color('🐶', 'cyan'), color('https://wa.me/62859193578139'))
    console.log(color('🐶', 'cyan'), color('ALVYN-BOTZ Is Online...', 'pink'))
    console.log(color('🐶', 'cyan'), color('Welcome Back Owner! Hope You Doing Well~', 'magenta'))
    console.log(color('🐶','red'), color('Thanks For Using ALVYN-BOTZ', 'white'))
    let { version, isLatest } = await fetchLatestBaileysVersion()
    const XeonBotInc = XeonBotIncConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['ALVYN-BOTZ-MD','Safari','1.0.0'],
        auth: state,
        version
    })
...
XeonBotInc.ev.on('groups.update', async pea => {
    ...
    // Get Profile Picture Group
       ...
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `${botname}`, wm_fatih, [])
       } else if(pea[0].announce == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `${botname}`, wm_fatih, [])
       } else if (pea[0].restrict == true) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `${botname}`, wm_fatih, [])
       } else if (pea[0].restrict == false) {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `${botname}`, wm_fatih, [])
       } else {
       XeonBotInc.send5ButImg(pea[0].id, `「 Group Settings Changed 」\n\nNama Grup telah diganti menjadi ${pea[0].subject}`, `${botname}`, wm_fatih, [])
     }
    })
